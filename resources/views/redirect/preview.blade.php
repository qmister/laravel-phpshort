    ...
    <div class="container">
        <div class="row h-100 justify-content-center align-items-center py-3">
            <div class="col-lg-12">
                ...
                <p class="text-center text-break text-dark font-weight-medium" dir="ltr">{{ str_replace(['http://', 'https://'], '', (($link->domain->name ?? config('app.url')) .'/'.$link->alias)) }}</p>
                ...
                @if($link->target_type)
                    ...
                    @if($link->target_type == 1 && $link->geo_target !== null)
                        @foreach($link->geo_target as $geo)
                            <p class="my-2 text-center text-break text-muted">{!! __('If the country is :country will redirect to :url', ['country' => '<span class="text-dark font-weight-medium">' . __(config('countries')[$geo->key]) . '</span>', 'url' => '<span class="text-dark font-weight-medium" dir="ltr">' . e($geo->value) . '</span>']) !!}</p>
                        @endforeach
                    @endif
                ...
                <p class="mb-5 text-center text-break text-muted">{!! __('Will redirect to :url', ['url' => '<span class="text-dark font-weight-medium" dir="ltr">' . e($link->url) . '</span>']) !!}</p>
